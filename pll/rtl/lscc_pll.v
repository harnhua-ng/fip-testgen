...
parameter                     SIM_FLOAT_PRECISION      = "0.625", // For simulation only
...
localparam FBK_MMD_PULS_CTL      = (FRAC_N_EN)? "0b0111" :
                                   (SS_EN)?     "0b0101" :
...
generate
  if (APB_EN == 1) begin : gen_apb
    ...
    if(APB_SOFT_REG_EN) begin : gen_csr
      wire    unused_w;
      pll_csr u_csr
      (
       // Inputs
       ...
       .pll_lmmi_offset                       ({unused_w,lmmi_offset_w[6:0]}),
  ...
  if(!(APB_EN && APB_SOFT_REG_EN)) begin : gen_dynports
  ...
  `else
   if(EN_REFCLK_MON) begin : gen_en_refclk_mon
     ...
     (/*AUTOINSTPARAM*/
      ...
      .FBK_CUR_BLE                           (FBK_CUR_BLE),           // Templated
      ...
      .FBK_PI_RC                             (FBK_PI_RC),             // Templated
      .FBK_PR_CC                             (FBK_PR_CC),             // Templated
      .FBK_PR_IC                             (FBK_PR_IC),             // Templated
      //.FLOAT_CP                              (),                      // Templated
...
`ifndef __RTL_MODULE__PLL_CSR__
`define __RTL_MODULE__PLL_CSR__
//==========================================================================
// Module : pll_csr
//==========================================================================
module pll_csr #
...
always @* begin
  case(csr_addr)
    ADR_REG_01  : csr_rdat_nxt = pll_register_01;
    default     : csr_rdat_nxt = 8'd0;
  endcase
end //--always @*--
  ...
  if(~rst_n_i) begin
    ...
    case(lmmi_sm_cs)
      ST_LMMI_WREQ : begin
        ...
        if(usr_lmmi_offset[7]) begin
          lmmi_sm_cs          <= ST_LMMI_DONE;
          lmmi_ready_r        <= 1'b1;
          pll_lmmi_request    <= 1'b0;
        end
        else begin
          if(pll_lmmi_ready) begin
            lmmi_sm_cs        <= ST_LMMI_DONE;
            lmmi_ready_r      <= 1'b1;
            pll_lmmi_request  <= 1'b0;
          end
      ...
      default : begin // ST_LMMI_IDLE
        lmmi_sm_cs            <= ST_LMMI_IDLE;
        lmmi_ready_r          <= 1'b0;
        lmmi_rvalid_r         <= 1'b0;
        pll_lmmi_request      <= 1'b0;
        if(usr_lmmi_request) begin
          pll_lmmi_request    <= ~usr_lmmi_offset[7];
          if(usr_lmmi_wr_rdn) begin
            lmmi_sm_cs        <= ST_LMMI_WREQ;
          end
          else begin
            lmmi_sm_cs        <= ST_LMMI_RREQ;
